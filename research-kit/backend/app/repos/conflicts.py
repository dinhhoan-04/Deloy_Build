...
class ConflictRepo:
    ...
    async def find_by_claim_pair(
        self, user_id: UUID, project_id: UUID, claim_id_a: UUID, claim_id_b: UUID
    ) -> Conflict | None:
    ...
    async def patch(self, user_id: UUID, conflict_id: UUID, *, resolution: str | None) -> Conflict:
        c = (
            await self.s.execute(
    ...
    async def confirm(
        self, user_id: UUID, conflict_id: UUID, accepted_claim_id: UUID
    ) -> tuple["Conflict", "InboxItem"]:
        ...
        for rid in rejected_ids:
            await self.s.execute(delete(Claim).where(Claim.id == rid, Claim.user_id == user_id))
