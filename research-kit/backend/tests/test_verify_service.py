...
async def test_verify_found_returns_verified_with_quote():
    with (
        patch("app.verify_service.lookup_paper", new_callable=AsyncMock, return_value=FAKE_PAPER),
...
async def test_verify_no_fulltext_returns_partial():
    paper_no_fulltext = PaperInfo(
        doi="10.1234/test",
        title="Effects of sleep",
        fulltext_url=None,
        authors=["Walker M"],
        year=2017,
    )
    with patch(
        "app.verify_service.lookup_paper", new_callable=AsyncMock, return_value=paper_no_fulltext
    ):
        result = await verify_claim(
            claim="Sleep improves memory", doi="10.1234/test", paper_title="Effects of sleep"
        )
