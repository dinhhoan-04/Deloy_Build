...
def test_build_doc_markdown_includes_topic_heading():
    md = build_doc_markdown(
        ...
        summary="Sleep is important.", claims=[_claim("Sleep helps memory.", None)],
        references=[], raw_by_tool={}
    )
...
def test_build_doc_markdown_groups_by_verify_score():
    md = build_doc_markdown(
        ...
        summary="S.", claims=[_claim("hi", 90), _claim("med", 70), _claim("low", 20)],
        references=[], raw_by_tool={}
    )
    hi, med, lo = md.index("High-confidence"), md.index("Medium-confidence"), md.index("Low-confidence")
    assert hi < med < lo
...
def test_build_doc_markdown_includes_verify_sections():
    md = build_doc_markdown(
        topic="x", date="d", tools_used=["elicit"], run_verify=True,
        summary="S.", claims=[_claim("hi", 90), _claim("med", 60), _claim("low", 20)],
        references=[], raw_by_tool={}
    )
...
def test_omits_verify_sections_when_run_verify_false():
    md = build_doc_markdown(
        topic="x", date="d", tools_used=["elicit"], run_verify=False,
        summary="S.", claims=[_claim("a", None)], references=[], raw_by_tool={}
    )
