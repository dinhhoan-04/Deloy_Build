...
async def test_detect_conflicts_creates_conflict_when_contradiction_found(async_session):
    ...
    claim_a = Claim(
        ...
        text="Exercise increases depression",
        doi=doi,
        paper_title="P1",
        site="pubmed",
    ...
    claim_b = Claim(
        user_id=user_id,
        project_id=project.id,
        text="Exercise reduces depression",
        doi=doi,
        paper_title="P1",
        site="pubmed",
    ...
    mock_provider.extract = AsyncMock(
        return_value={"contradictions": [{"candidate_id": str(claim_a.id), "contradicts": True}]}
    )
    ...
    conflicts = list(
        (
            await async_session.execute(select(Conflict).where(Conflict.project_id == project.id))
        ).scalars()
    )
...
async def test_detect_conflicts_no_conflict_when_no_contradiction(async_session):
    ...
    mock_provider = AsyncMock()
    mock_provider.extract = AsyncMock(
        return_value={"contradictions": [{"candidate_id": str(claim_a.id), "contradicts": False}]}
    )
...
async def test_detect_conflicts_no_duplicate(async_session):
    ...
    existing = Conflict(
        ...
        paper_title="P",
        sides=[
            {"claim_id": str(claim_a.id), "label": "A", "quote": "q"},
...
async def test_detect_conflicts_ignores_hallucinated_candidate_ids(async_session):
    ...
    async_session.add_all([new_claim, cand])
    await async_session.flush()
...
async def test_detect_conflicts_sets_checked_at_on_llm_success(async_session):
    ...
    refreshed = (
        await async_session.execute(select(Claim).where(Claim.id == claim_b.id))
    ).scalar_one()
...
async def test_detect_conflicts_sets_checked_at_when_all_pairs_already_exist(async_session):
    ...
    existing = Conflict(
        ...
        paper_title="P",
        sides=[
            {"claim_id": str(new_claim.id), "label": "A", "quote": "qn"},
    ...
    refreshed = (
        await async_session.execute(select(Claim).where(Claim.id == new_claim.id))
    ).scalar_one()
    assert refreshed.conflicts_checked_at is not None
...
async def test_detect_conflicts_marks_checked_on_llm_timeout(async_session):
    ...
    with (
        patch("app.routers.claims._make_provider", return_value=mock_provider),
