...
async def test_trusted_resolvable_http_ok_scores_100():
    with patch("app.services.tier1.link_validator._check_http", new_callable=AsyncMock, return_value=True), \
         patch("app.services.tier1.link_validator._check_resolvable", new_callable=AsyncMock, return_value=True), \
...
async def test_http_ok_only_scores_50():
    with patch("app.services.tier1.link_validator._check_http", new_callable=AsyncMock, return_value=True), \
         patch("app.services.tier1.link_validator._check_resolvable", new_callable=AsyncMock, return_value=False), \
...
async def test_http_fail_scores_zero():
    with patch("app.services.tier1.link_validator._check_http", new_callable=AsyncMock, return_value=False), \
         patch("app.services.tier1.link_validator._check_resolvable", new_callable=AsyncMock, return_value=False), \
         patch("app.services.tier1.link_validator._check_trusted_domain", return_value=False):
...
async def test_timeout_scores_zero_status_timeout():
    with patch("app.services.tier1.link_validator._check_http", new_callable=AsyncMock, side_effect=TimeoutError), \
         patch("app.services.tier1.link_validator._check_resolvable", new_callable=AsyncMock, return_value=False), \
         patch("app.services.tier1.link_validator._check_trusted_domain", return_value=False):
        result = await validate_link(ref_id="1", url=DEAD_URL)
    assert result.score == 0
