...
async def test_inbox_add_list_delete(db_engine):
    ...
    async with db_engine() as s:
        u = User(google_sub="g1", email="g1@x")
        s.add(u)
        await s.flush()
        ...
        await s.flush()
        c = Claim(
            user_id=u.id, project_id=p.id, text="t", site="elicit", status=ClaimStatus.PENDING.value
        )
        s.add(c)
        await s.commit()
        await s.refresh(c)
        ...
        await repo.remove(u.id, item.id)
        await s.commit()
...
async def test_inbox_endpoint_roundtrip(client_dev_alice):
    ...
    r = await client_dev_alice.post(
        "/v1/claims/batch", json={"project_id": pid, "claims": [{"text": "c", "site": "elicit"}]}
    )
...
async def test_inbox_patch_not_found(client_dev_alice):
    ...
    r = await client_dev_alice.patch(
        f"/v1/inbox/{fake_id}", json={"archived_at": datetime.now(timezone.utc).isoformat()}
    )
...
async def test_inbox_remove_keeps_claim(db_engine):
    ...
    async with db_engine() as s:
        u = User(google_sub="g_keep", email="g_keep@x")
        s.add(u)
        await s.flush()
        p = Project(user_id=u.id, name="P")
        s.add(p)
...
async def test_inbox_bulk_patch_preserves_input_order(client_dev_alice):
    ...
    now = datetime.now(timezone.utc).isoformat()
    r = await client_dev_alice.patch(
        "/v1/inbox/bulk", json={"ids": reversed_ids, "archived_at": now}
    )
    assert r.status_code == 200
